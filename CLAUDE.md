...
3. **`<main id="inicio">`** — the page content, as numbered sections each with `id` anchors matching the nav links:
   - `.hero` — full-bleed hero image (`Hero2_esslp8.png`, a single three-scene panorama: solar rooftop on the left, hardware-diagnostics bench in the center, software/AI screens on the right), shown **clean, with no overlay labels** — the image's blue light-flow connects the three scenes and the divisions live in the nav instead (see below). The `.hero__side` label CSS still exists because the careers hero and the warranty landing use it. On phones (≤640px) NO hero photo is shown at all: every page's hero becomes a dark card (pill-styled `.hero__spec` chips with copper dots, full-width rounded CTAs, a copper glow line at the bottom edge) closed by a `.hero__intro` strip (short promise line + thumbnail(s) of the page's hero image; the homepage shows two thumbnails, subpages one). The old `.fig__split` mobile split-image mechanism was removed site-wide. The nav collapses to the burger from 1120px down: the grown desktop nav overflows on tablets. Each capacidades division has its own dedicated image (set in `styles.css`): `.cap--e` `energia_r29mxp.png`, `.cap--t` `software_zmzqjd.png`, `.cap--w` `soporte_ta7wxd.png`.
...
- Each hero reuses the shared `.hero` structure; the "División 0X" label is **bottom-centered** via `.hero__side--w` (desktop only: on phones the hero photo is hidden and shows as the `.hero__intro` thumbnail instead). Division hero images are the same ones used by the homepage `.cap__img` blocks (`energia_r29mxp.png`, `software_zmzqjd.png`, `soporte_ta7wxd.png`).
